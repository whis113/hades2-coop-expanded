...
文档状态：2026-07-14 已同步 v0.2 外部测试包、独立阿卡那和锤子候选修复状态。
...
```text
E:\hades2coop\workplace\hades2-coop-expanded
```
...
1. 完全退出 Hades II。
2. 解压 `Hades2Coop-v0.2-TestBuild.zip`，保持 `Hades2CoopInstaller.exe` 与 `TN_CoopMod` 在同一目录。
3. 运行安装器，选择 `<Hades II>\Ship\Hades2.exe`。
4. 点击“安装 / 更新 Mod”。

安装器仅写入 `<Hades II>\Content\Mods\TN_CoopMod`；可在同一界面卸载，不会修改游戏本体或删除存档。详细中英文测试说明见发布包中的 `TESTER_README.md`。
...
```text
workplace/hades2-coop-expanded/game/scripts/config.lua
```
...
| `Debug.ArcanaFullUnlockRepair = true` | 临时阿卡那界面救援开关，后续应回撤。 |
| `Debug.ArcanaMaxLevelRepair = true` | 临时阿卡那等级救援开关，后续应回撤。 |

## 项目结构

| 路径 | 用途 |
|------|------|
| `workplace/hades2-coop-expanded/` | 当前正在开发的 Hades II 本地双人 mod。 |
| `MYcoopProject/hades2-coop-github-clone/` | 从 `whis113/hades2-coop` clone 的历史验证副本，不作为当前开发目录。 |
...
- 所有 mod trace 现自动带地点标签，例如 `[Underworld-Layer1-Erebus-CombatRoom-F_Combat12]`；详见 `PROJECT_LOG.md`。
...
- `ArcanaFullUnlockRepair` 和 `ArcanaMaxLevelRepair` 是临时存档/界面救援方案，不应作为长期设计方向。
- 详细开发过程不要堆在 README，写入 `PROJECT_LOG.md`。
...
```text
C:\Users\haoxu\Saved Games\Hades II\TN_CoopMod.log
```
...
```text
workplace/hades2-coop-expanded/LICENSE.txt
```
...
- 已知未修复：run 回 Hub 后的 HP/MP HUD 延迟刷新；`F_PreBoss` 中扎格列欧斯挑战奖励尚未双份；哀悼原野 `TalentDrop` 双份仍待专项验证；旧版阿卡那全解锁/满级救援开关尚未回撤。
...
- 更新工作副本路径为 `E:\hades2coop\MYcoopProject\hades2coop-mode`。
- 修复并整理开发部署脚本。
