...
文档状态：2026-07-17 已同步 v0.2.4 外部测试包与过门基础 MP 回满修复状态。
...
- 过门基础 MP 回满：任一存活玩家过门时，P1/P2 都按各自可用 MP 上限执行原版回满；不依赖阿卡那或道具，死亡玩家不参与。
...
1. 完全退出 Hades II。
2. 先阅读压缩包根目录的 `README.md`，再解压 `Hades2Coop-v0.2.4-TestBuild.zip`，保持 `Hades2CoopInstaller.exe`、`TN_CoopMod` 与 `Dependencies` 在同一目录。
...
- v0.2.4 发布包命名为 `Hades2Coop-v0.2.4-TestBuild.zip`；内含自包含安装器、完整 Mod、敌人生命值工具、MIT 许可证和发布包根目录的中英文 `README.md`；测试说明要求保持 `TN_CoopMod.log` 的实时终端开启。
